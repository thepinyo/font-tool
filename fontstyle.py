...
#
# Usage:
#   python3 fontname.py [FONT STYLE NAME] [FONT PATH 1] <FONT PATH ...>
#
# Notes:
...
def main(argv):
    ...
    if len(argv) < 2:
        ...
        sys.stderr.write(
            f"Usage: python3 fontname.py [FONT FAMILY STYLE] [FONT PATH 1] <FONT PATH ...>{os.linesep}"
        )
        sys.exit(1)

    # begin parsing command line arguments
    try:
        font_style = str(argv[0])  # the first argument is the new typeface style
        nameID2_string = font_style
        print(f"[input][0][font style]: '{font_style}'")
    ...
    print(f"[path list] '{font_path_list}'")
    ...
    for font_path in font_path_list:
        ...
        print(f"[name list] '{namerecord_list}'")

#        font_style = ""

        # Font Name
        # nameID: 1, 4, 6, 16
        # get font name from existing
        # determine font name for this file path from name record
        font_name = ""
        print(f"- Read Data all the record -")
        for record in namerecord_list:
            if record.nameID == 1:
                ...
                print(f"[{record.nameID}] '{str(record)}'")
                
            
        postscript_font_name = font_name.replace(" ", "")
        nameID4_string = f"{postscript_font_name} {font_style}"
        nameID6_string = f"{postscript_font_name}-{font_style.replace(' ', '')}"

        print(f"[nameID][01] '{nameID1_string}'")
        print(f"[nameID][04] '{nameID4_string}'")
        print(f"[nameID][06] '{nameID6_string}'")
        print(f"[nameID][16] '{nameID16_string}'")
        
        # test that a style name was found in the OpenType tables of the font
        if len(font_name) == 0:
            sys.stderr.write(
                f"[fontname.py] Unable to detect the font name from the OpenType name table in '{font_path}'. {os.linesep}"
            )
...
#            nameID6_string = f"{postscript_font_name}-{style.replace(' ', '')}"
        ...
        try:
            tt.save(font_path)
            print(f"[OK] Updated '{font_path}' with the style '{font_style}'")
        except Exception as e:
            sys.stderr.write(
